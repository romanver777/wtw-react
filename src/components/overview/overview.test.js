...
describe("OverView component", () => {
  ...
  const TRenderer = TestRenderer.create(<Overview {...props} />);

  it("renders correctly", () => {
    expect(TRenderer.toJSON()).toMatchSnapshot();
  });
  it("renders 1 div with className 'movie-rating'", () => {
    const cName = "movie-rating";
  ...
  it("should be div with rating", () => {
    const cName = "movie-rating__score";

    expect(
      TRenderer.root.findByProps({
        className: `${cName}`,
      }).props.children
    ).toEqual(props.film.rating);
  });
  it("renders 1 div with className 'movie-card__text'", () => {
    const cName = "movie-card__text";

    expect(
      TRenderer.root.findAllByProps({ className: `${cName}` })
    ).toHaveLength(1);
  });
  it("should be div with description", () => {
    const cName = "movie-card__text";

    expect(
      TRenderer.root.findByProps({ className: `${cName}` }).props.children[0]
        .props.children
    ).toEqual(props.film.description);
  });
});
